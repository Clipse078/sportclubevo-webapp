...
/** High resolution for PO pixel review (SWISS-01E4C). */
const DPI = 300;
...
async function main() {
  ...
  const fullPrefix = path.join(ARTIFACT_DIR, "swiss-01e4c-a4-full");
  ...
  await cropPng(fullPng, path.join(ARTIFACT_DIR, "swiss-01e4c-creative-area-210x192.png"), {
    x: 0,
    y: 0,
    width: pageWidthPx,
  ...
  await cropPng(fullPng, path.join(ARTIFACT_DIR, "swiss-01e4c-payment-section-210x105.png"), {
    x: 0,
  ...
  await cropRegion("acknowledgement", "swiss-01e4c-thankyou-brand-crop.png", 5);
  ...
  console.log("SWISS-01E4C artifacts:", ARTIFACT_DIR);
