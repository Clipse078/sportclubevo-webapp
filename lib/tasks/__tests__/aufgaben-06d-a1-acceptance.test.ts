...
vi.mock("@/lib/workspace/document-access", () => ({
  assertWorkspaceDocumentLinkable: refMocks.assertLinkable,
  resolveWorkspaceDocumentPresentations: refMocks.resolvePresentations,
...
describe("AUFGABEN-06D-A1 schema & migration (R1–R11)", () => {
  ...
  it("R2 references WorkspaceDocument not WorkspaceDocumentVersion", () => {
    const block =
      read("prisma/schema.prisma").match(/model TaskDocumentReference[\s\S]*?^}/m)?.[0] ?? "";
    expect(block).toMatch(/document\s+WorkspaceDocument @relation/);
    expect(block).not.toMatch(/WorkspaceDocumentVersion/);
  });

  it("R3/R4 tenant + unique task/document pair", () => {
    const schema = read("prisma/schema.prisma");
    expect(schema).toMatch(/tenantId\s+String/);
    expect(schema).toMatch(/@@unique\(\[taskId, documentId\]\)/);
  });
...
describe("AUFGABEN-06D-A1 metadata & errors (A3–A4, R17, R29, R43)", () => {
  ...
  it("A3 restricted reference DTO exposes no confidential metadata fields", async () => {
    ...
    refMocks.resolvePresentations.mockResolvedValue(
      new Map([[DOC, { access: "restricted", documentId: DOC }]]),
    );
    ...
    expect(rows[0]?.presentation).toEqual({ access: "restricted", documentId: DOC });
  });
  ...
  it("A4 link failures use one generic message for missing/foreign/unreadable", async () => {
    ...
    for (const reason of ["missing", "foreign-tenant", "permission-denied", "archived"]) {
      refMocks.assertLinkable.mockRejectedValueOnce(new Error(reason));
      await expect(linkTaskDocument(ctx, TASK, DOC)).rejects.toMatchObject({
    ...
    const rows: TaskDocumentReferenceDto[] = [
      {
        ...
        presentation: { access: "restricted", documentId: DOC },
      },
    ];
...
describe("AUFGABEN-06D-A1 integrity & side effects (A5–A10, R15, R24, R30–R38)", () => {
  ...
  it("A10 Phase-10 seam — reference service has no direct workspace permission checks", () => {
    ...
    expect(source).toMatch(/@\/lib\/workspace\/document-access/);
  });
  ...
  it("R24 unlink audit stores documentId only", () => {
    const source = read("lib/tasks/task-document-reference-service.ts");
    const unlink = source.split("TASK_DOCUMENT_UNLINKED")[1] ?? "";
    expect(unlink).toMatch(/afterJson: \{ documentId/);
...
describe("AUFGABEN-06D-A1 access loss & deep link (R21, R40)", () => {
  ...
  it("document access loss hides metadata while reference remains listed", async () => {
    ...
    expect(rows[0]?.presentation.access).toBe("restricted");
    if (rows[0]?.presentation.access === "restricted") {
      expect(rows[0].presentation).not.toHaveProperty("href");
      expect(rows[0].presentation).not.toHaveProperty("title");
    }
  });
...
describe("AUFGABEN-06D-A1 wiring (R41–R42)", () => {
  it("R41 server actions delegate to reference service", () => {
    ...
    expect(actions).toMatch(/await linkTaskDocument\(ctx, taskId, documentId\)/);
  });
