...
function computeIdentityViolations(
  ...
  if (
    ...
    runtime.vercelTargetEnv !== null &&
    runtime.vercelTargetEnv.toLowerCase() !== "acceptance"
  ) {
    violations.push(
      `Environment classified as acceptance but VERCEL_TARGET_ENV=${runtime.vercelTargetEnv}.`,
    );
  }
...
export function resolveDeploymentIdentity(
  ...
  const identityViolations = computeIdentityViolations(runtime);
