...
describe("runDeploymentPreflight", () => {
  ...
  describe("environment/database identity mismatch", () => {
    it("fails when APP_ENV contradicts VERCEL_ENV=preview on a plain Preview", () => {
    ...
    it("fails when Preview points to STAGE database host", () => {
      const result = runDeploymentPreflight({
        ...VALID_PREVIEW_ENV,
      ...
      expect(result.pass).toBe(false);
      expect(
        result.violations.some((v) => v.code === "PREVIEW_TARGETING_STAGE_DB"),
      ).toBe(true);
    });
  });
