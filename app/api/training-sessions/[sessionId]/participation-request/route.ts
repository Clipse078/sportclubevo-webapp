...
import { updateTrainingSessionParticipationRequestConfig } from "@/lib/participation/participation-request-config-service";
import { prisma } from "@/lib/db/prisma";

type Params = { params: Promise<{ sessionId: string }> };

export async function PATCH(request: NextRequest, { params }: Params) {
  const access = await requireApiAnyPermission([PERMISSIONS.TRAININGS_MANAGE]);
  if (!access.ok) {
